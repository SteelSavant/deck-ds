...
pub fn get_display_info() -> Option<Vec<DisplayValues>> {
    ...
    let file = "/sys/class/drm/card0-DP-1/edid";
    let bytes = std::fs::read(file).ok()?;

    let edid = edid_rs::parse(&mut Cursor::new(bytes)).ok()?;
    ...
    let standard = edid
        ...
        .into_iter()
        .map(|v| DisplayValues {
            width: v.horizontal_resolution,
            height: (v.horizontal_resolution as f32 / v.aspect_ratio) as u16,
